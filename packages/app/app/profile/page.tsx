...
function CollectionTile({
  ...
  const priceLabel = item.marketValueUsd > 0 ? formatUsd(item.marketValueUsd) : "Free";
    ...
    >
      {imageUrl && (
        <div className="relative">
          <img
            src={imageUrl}
            alt={description || `Sticker #${item.tokenId.toString()}`}
            className={`w-full object-cover${!item.isApproved ? " opacity-50" : ""}`}
          />
      ...
      {!imageUrl && !hasText && isMetadataLoading && (
      ...
      {!imageUrl && !hasText && !isMetadataLoading && (
        <div className="relative aspect-square flex items-center justify-center bg-secondary">
...
export default function ProfilePage() {
  ...
  const { holdings, collection, totalCoinValueUsd, totalCollectionValueUsd, isLoading } = useUserProfile(address);

  // Sparkline data for coin holdings
  ...
  const totalValueUsd = usdcNum + totalCoinValueUsd + totalCollectionValueUsd;
      ...
      >
        ...
        <div className="px-4 pb-2">
          <div className="mb-3">
            ...
            <p className="text-[13px] text-muted-foreground mt-1">Your portfolio, coin balances, and sticker collection</p>
          </div>
            ...
            <>
              {collection.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <div className="text-[13px] text-muted-foreground mb-4">
                    Your collection will show the stickers you own
                  </div>
                  <Link
                    href="/explore"
                    className="px-4 py-2 rounded-none bg-white text-black text-[13px] font-semibold font-display hover:bg-zinc-200 transition-colors"
